...
7. Follow [these instructions](https://github.com/ryanoasis/nerd-fonts#font-installation) to get patched fonts with neat icons for the vim-devicons plugin. *This is optional. If you don't want to use devicons, remove it from the .vimrc file*
8. Install some linters `npm install -g eslint`, `npm install -g babel-eslint`, `npm install -g eslint-plugin-react`
9. Call `npm install` in the `bundle/tern_for_vim` directory to install Tern. 
10. Install YouCompleteMe. Follow [these instructions](https://github.com/Valloric/YouCompleteMe#ubuntu-linux-x64-super-quick-installation), and be sure to use the `./install.py --tern-completer` flag when you install!
